...
orden.getProductoxOrden = function getProductoxOrden(id) {
  return new Promise(resolve => {
    db.query(
      ...
      'FROM productosxorden AS p WHERE p.id_orden = ' + id,
      (err, res) => {
        resolve(res);
      })
  })
...
orden.insertarOrden = function insertarOrden(order) {
  return new Promise(resolve => {
    db.query('INSERT INTO ordenes SET ?',
      order,
      (err, res) => {
        resolve(res)
      })
...
orden.insertarProducto = function insertarProducto(id, product) {
  return new Promise(resolve => {
    db.query('INSERT INTO productosxorden SET `id_orden` = ' + id + ', ?',
      product,
      (err, res) => {
...
orden.actualizarOrden = function actualizarOrden(id, order) {
  ...
  return new Promise(resolve => {
    db.query('UPDATE ordenes SET ? WHERE id = ' + id,
      order,
      (err, res) => {
...
orden.actualizarProducto = function actualizarProducto(id, product) {
  ...
  return new Promise(resolve => {
    db.query('UPDATE productosxorden SET ? WHERE id_orden = ' + id +
      ' AND id_producto = \'' + product.id_product + '\'',
      product,
      (err, res) => {
...
orden.borrarProductos = function borrarProductos(id) {
  return new Promise(resolve => {
    db.query('DELETE FROM productosxorden WHERE id_orden = ' + id,
      (err, res) => {
        resolve(res)
      })
  })
...
orden.borrarProducto = function borrarProducto(code, order) {
  return new Promise(resolve => {
    db.query('DELETE FROM productosxorden WHERE id_producto = \'' + code + '\' AND id_orden = ' + order ,
      (err, res) => {
        console.log(err)
        resolve(res)
      })
